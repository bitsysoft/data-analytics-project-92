...
-- В задании не сказано отобрать уникальных покупателей, поэтому distinct
-- не использовался
...
-- INNER JOIN потому, что покупки непонятных продавцов мы учитывать не будем
...
ORDER BY tbl.day_num, tbl.seller;
...
res AS (
    ...
    WHERE
        ...
        AND NOT EXISTS (
            SELECT 1
            FROM t AS t2
            WHERE t2.customer_id = t.customer_id AND t2.sale_date < t.sale_date
        )
)
...
SELECT
    ...
    (
        ...
        FROM customers AS c
        WHERE c.customer_id = res.customer_id
    ) AS customer,
    (
        SELECT concat(e.first_name, ' ', e.last_name)
        FROM employees AS e
        WHERE e.employee_id = res.sales_person_id
    ) AS seller
FROM res
